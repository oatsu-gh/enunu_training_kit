...
if [ -d train/conf/synthesis ]; then
    ext="--config-dir train/conf/synthesis"
else
    ext=""
fi
...
for s in ${testsets[@]}; do
    for input in label_phone_score label_phone_align; do
        ...
        xrun nnsvs-synthesis $ext sample_rate=$sample_rate \
            ...
            in_dir=target/data/acoustic/$input/ \
            out_dir=$expdir/synthesis_timelag_duration_acoustic/$s/${acoustic_eval_checkpoint/.pth/}/$input \
